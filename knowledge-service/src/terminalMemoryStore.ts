/**
 * TerminalMemoryStore - SQLite WAL-based memory management for dual-session architecture
 *
...
// ─── TerminalMemoryStore Class ───────────────────────────────────────────────

export class TerminalMemoryStore {
  private db: Database.Database;
  private terminal: string;

  /**
   * Initialize TerminalMemoryStore for a terminal
  ...
  constructor(terminal: string) {
    ...
    logger.info(`[TerminalMemoryStore] Initialized for terminal: ${terminal} (WAL mode)`);
  }
  ...
  close(): void {
    this.db.close();
    logger.info(`[TerminalMemoryStore] Closed database for terminal: ${this.terminal}`);
  }
  ...
  vacuum(): void {
    this.db.exec('VACUUM');
    logger.info(`[TerminalMemoryStore] Vacuumed database for terminal: ${this.terminal}`);
  }
}

/** Backwards-compatible alias for TerminalMemoryStore. */
export const MemoryStore = TerminalMemoryStore;
export type MemoryStore = TerminalMemoryStore;

// ─── Factory Function ─────────────────────────────────────────────────────────

/**
 * Get or create a TerminalMemoryStore instance for a terminal
 * @param terminal - Terminal name
   * @returns TerminalMemoryStore instance
 */
export function getMemoryStore(terminal: string): TerminalMemoryStore {
  return new TerminalMemoryStore(terminal);
}
