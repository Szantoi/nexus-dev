/**
 * ftsMemoryStore.ts — FTS5-based memory system for SpaceOS
 *
...
function initSchema(): void {
  ...
  log('ftsMemoryStore', 'Database schema initialized (with tier support)');
}
...
export async function saveMemory(input: MemoryInput): Promise<number> {
  ...
  log('ftsMemoryStore', `Saved memory #${result.lastInsertRowid} (${input.type}/${input.source}, tier=hot)`);
...
export async function saveTieredMemory(input: TieredMemoryInput): Promise<TieredMemory> {
  ...
  log('ftsMemoryStore', `Saved tiered memory #${id} (${input.type}/${input.source}, tier=${input.tier})`);
...
export async function promoteMemory(id: number, newTier: MemoryTier, reason: string): Promise<void> {
  ...
  log('ftsMemoryStore', `Promoted memory #${id} from ${currentTier} → ${newTier} (reason: ${reason})`);
}
...
export function runSalienceDecay(): number {
  ...
  if (totalAffected > 0) {
    log('ftsMemoryStore', `Decay applied to ${totalAffected} memories (tier-specific rates)`);
  }
...
export function cleanupMemories(options: { minSalience?: number; archiveInsteadOfDelete?: boolean } = {}): number {
  ...
  if (deleted > 0) {
    log('ftsMemoryStore', `Cleaned up ${deleted} memories`);
  }
...
export function closeMemoryStore(): void {
  if (db) {
    db.close();
    db = null;
    log('ftsMemoryStore', 'Database connection closed');
  }
}
